 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`ozbcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`ozbcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'ozbcore-' + name : 'ozbcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/ozbcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external ozbcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:ozbcore-lib -o ozbcore-lib.js';
    }
  ...
  gulp.task('release:push', function(cb) {
    git.push('ozosbittglobal'', 'master', {
      args: ''
    }, cb);
  });
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('ozosbittglobal'', name, cb);
    });
  });
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push ozosbittglobal $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
